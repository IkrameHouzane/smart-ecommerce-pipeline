...
# ── Page title ────────────────────────────────────────────────────────────────
st.markdown(
    '<div class="pg-main">Dashboard Overview</div>'
...
c4.markdown(
    kpi(f"{len(anomalies)}", "Anomalies DBSCAN", "#ff6b6b"), unsafe_allow_html=True
)

# ── Insight narrative ─────────────────────────────────────────────────────────
best = shops.iloc[0]["shop_name"]
top_cat = (
    scored["category_clean"].value_counts().index[0]
...
c1.markdown(
    ins(
        ...
        f"<strong>{pct_promo:.1f}%</strong> carry a discount signal.",
    ),
    unsafe_allow_html=True,
)
c2.markdown(
...
st.markdown(
    sec(
        "Distribution des prix",
        "Histogram capped at $300 · and price tier segmentation.",
    ),
    unsafe_allow_html=True,
)
c1, c2 = st.columns(2)
with c1:
    fig = px.histogram(
        ...
        x="price",
        nbins=50,
        color_discrete_sequence=["#e8d5a3"],
...
st.markdown(
    sec(
        "Score moyen par boutique",
        "Composite score averaged across all products per shop.",
    ),
    unsafe_allow_html=True,
)
...
fig3 = px.bar(
    ...
    template="plotly_dark",
    title="Score composite moyen par boutique",
)
...
st.markdown(
    sec(
        "Statut du Pipeline Kubeflow",
        "All 8 components executed successfully on Minikube · KFP 2.4.1",
    ),
    unsafe_allow_html=True,
)
steps = [
