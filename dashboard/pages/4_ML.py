...
def _get_metric(data, key, fallback):
    return (
        data.get("classification_report", {}).get("weighted avg", {}).get(key, fallback)
    )
...
# ── Page title ────────────────────────────────────────────────────────────────
st.markdown(
    '<div class="pg-main">ML Models</div>'
...
c1.markdown(
    kpi(f"{rf_acc * 100:.1f}%", "Random Forest", "#a8e6cf"), unsafe_allow_html=True
)
c2.markdown(kpi(f"{xgb_acc * 100:.1f}%", "XGBoost", "#4ecdc4"), unsafe_allow_html=True)
c3.markdown(
    kpi(f"{cv_mean * 100:.1f}%", "Cross-Val RF", "#e8d5a3"), unsafe_allow_html=True
)
...
st.markdown(
    ins(
        ...
        f"cluster strongly in the premium tier.",
    ),
    unsafe_allow_html=True,
)
...
st.markdown(
    sec(
        ...
        "Gini impurity decrease — how much each feature reduces uncertainty in the classification.",
    ),
    unsafe_allow_html=True,
)
...
fig = px.bar(
    ...
    orientation="h",
    color=import_col,
    text=fi_s[import_col].round(3),
    color_continuous_scale=["#1a1a28", "#2a4a3a", "#4ecdc4", "#a8e6cf"],
    template="plotly_dark",
    title="Top features par importance (Gini impurity decrease)",
)
...
st.markdown(
    sec(
        "Comparaison RF vs XGBoost",
        "Accuracy bar and radar of Precision / Recall / F1 / Cross-Val metrics.",
    ),
    unsafe_allow_html=True,
)
c1, c2 = st.columns(2)
with c1:
    ...
    fig2.update_layout(
        **pl(
            title="Accuracy",
            yaxis=dict(range=[0, 1.06], gridcolor="#1a1a28"),
            height=320,
        )
    )
...
with c2:
    ...
    fig3.add_trace(
        go.Scatterpolar(
            r=[rf_acc, rf_precision, rf_recall, rf_f1, cv_mean],
            theta=cats,
            fill="toself",
            name="Random Forest",
            line_color="#4ecdc4",
    ...
    fig3.add_trace(
        go.Scatterpolar(
            r=[xgb_acc, xgb_precision, xgb_recall, xgb_f1, xgb_acc],
            theta=cats,
            fill="toself",
            name="XGBoost",
            line_color="#e8d5a3",
    ...
    fig3.update_layout(
        polar=dict(
            ...
            angularaxis=dict(
                gridcolor="#1e1e2e", tickfont=dict(color="#7070a0", size=10)
            ),
        ),
        paper_bgcolor="#0f0f18",
        font_color="#7070a0",
        title=dict(
            text="Radar Métriques",
...
st.markdown(
    sec(
        ...
        "Per-class Precision / Recall / F1 on the 20% test set (433 products).",
    ),
    unsafe_allow_html=True,
)
st.code(
...
st.markdown(
    sec(
        "Distribution des classes (price_tier)",
        "Training + test set — 2 165 products across 3 price tiers.",
    ),
    unsafe_allow_html=True,
)
