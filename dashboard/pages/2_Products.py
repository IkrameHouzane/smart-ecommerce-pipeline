...
# ── Page title ────────────────────────────────────────────────────────────────
st.markdown(
    '<div class="pg-main">Product Rankings</div>'
...
c1.markdown(
    ins(
        ...
        f"<strong>{avg_r:.2f}</strong> in rating.",
    ),
    unsafe_allow_html=True,
)
c2.markdown(
    ins(
        ...
        f"while <strong>{pct_disc:.1f}%</strong> carry a discount signal.",
        "#ffd93d",
    ),
    unsafe_allow_html=True,
)
c3.markdown(
    ins(
        ...
        f"useful as a broad leaderboard or a niche buying slice.",
        "#ff6b6b",
    ),
    unsafe_allow_html=True,
)
...
st.markdown(
    sec(
        "Top 10 Produits",
        "Ordered by composite score — 4 decimal precision to show real differentiation.",
    ),
    unsafe_allow_html=True,
)
...
fig = px.bar(
    ...
    title="Top 10 produits par score composite",
...
c1, c2, c3 = st.columns(3)
c1.markdown(
    met(f"${top10['price'].mean():.2f}", "Prix moyen Top-10", "#e8d5a3"),
    unsafe_allow_html=True,
)
c2.markdown(
    met(f"{top10['rating_filled'].mean():.2f} ★", "Note moyenne Top-10", "#4ecdc4"),
    unsafe_allow_html=True,
)
c3.markdown(
    met(f"{top10['discount_pct'].mean():.1f}%", "Remise moyenne Top-10", "#ffd93d"),
    unsafe_allow_html=True,
)
...
st.markdown(
    sec(
        "Score vs Prix",
        "Each dot is a product — ideal products cluster top-left: high score, low price.",
    ),
    unsafe_allow_html=True,
)
...
st.markdown(
    sec(
        ...
        "Select a segment to inspect the top ranked products.",
    ),
    unsafe_allow_html=True,
)
cats = (
    sorted(catk["category_clean"].unique().tolist())
    if "category_clean" in catk.columns
    else []
)
if cats:
    ...
    st.dataframe(
        cat_df[
            ["title", "shop_name", "price", "rating_filled", "composite_score"]
...
st.dataframe(
    df[["title", "shop_name", "price", "rating_filled", "composite_score"]]
