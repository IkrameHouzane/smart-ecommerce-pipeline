...
# ── Page title ────────────────────────────────────────────────────────────────
st.markdown(f'<div class="pg-main">Shop Analysis</div>'
...
c1.markdown(kpi(top1["shop_name"],                      "Meilleure boutique", "#e8d5a3"), unsafe_allow_html=True)
c2.markdown(kpi(f"{top1['score_moyen']:.3f}",           "Score #1",           "#4ecdc4"), unsafe_allow_html=True)
c3.markdown(kpi(f"{shops['nb_produits'].sum():,}",      "Produits total",     "#a8e6cf"), unsafe_allow_html=True)
c4.markdown(kpi(f"{shops['note_moyenne'].mean():.2f} ★","Note moyenne",       "#ffd93d"), unsafe_allow_html=True)

# ── Insight narrative ─────────────────────────────────────────────────────────
gap  = top1["score_moyen"] - shops.iloc[-1]["score_moyen"]
top3 = ", ".join(shops.head(3)["shop_name"].tolist())
st.markdown(ins("Competitive Landscape",
    ...
    f"indicating meaningful spread in catalog quality across the niche."), unsafe_allow_html=True)

# ── Classement global ─────────────────────────────────────────────────────────
st.markdown(sec("Classement global des boutiques",
    "Composite score averaged across all products per shop — ordered best to worst."), unsafe_allow_html=True)
sh = shops.sort_values("score_moyen", ascending=True)
fig = px.bar(sh, x="score_moyen", y="shop_name", orientation="h",
    color="score_moyen", text=sh["score_moyen"].round(3),
    color_continuous_scale=["#1a1a28", "#3d2b1a", "#ffd93d", "#ff6b6b"],
    template="plotly_dark", title="Score moyen par boutique",
...
st.markdown(sec("Métriques détaillées", "Average price and product distribution."), unsafe_allow_html=True)
c1, c2 = st.columns(2)
with c1:
    sh2 = shops.sort_values("prix_moyen", ascending=False)
    fig2 = px.bar(sh2, x="shop_name", y="prix_moyen", color="prix_moyen",
        text=sh2["prix_moyen"].round(1),
        color_continuous_scale=["#1a1a28", "#2a2a4a", "#4ecdc4", "#e8d5a3"],
        template="plotly_dark", title="Prix moyen par boutique (€)")
    fig2.update_traces(textposition="outside")
    fig2.update_layout(**PLOT, height=360, showlegend=False,
        coloraxis_showscale=False, xaxis_tickangle=-30)
    st.plotly_chart(fig2, use_container_width=True)
with c2:
    fig3 = px.pie(shops, values="nb_produits", names="shop_name",
...
# ── Notes moyennes ────────────────────────────────────────────────────────────
st.markdown(sec("Notes moyennes par boutique",
    "Mean rating — real where available, imputed by tier median elsewhere."), unsafe_allow_html=True)
sh3 = shops.sort_values("note_moyenne", ascending=False)
fig4 = go.Figure(go.Bar(
    x=sh3["shop_name"], y=sh3["note_moyenne"],
    marker_color=[f"rgba(232,213,163,{0.3+0.7*(v-4.0)/1.2})" for v in sh3["note_moyenne"]],
    text=[f"★ {v:.2f}" for v in sh3["note_moyenne"]], textposition="outside"))
fig4.update_layout(**{**PLOT, "yaxis": dict(range=[0, 5.5], gridcolor="#1a1a28")},
    title="Notes moyennes (★/5)", height=350, xaxis_tickangle=-30)
st.plotly_chart(fig4, use_container_width=True)

# ── Remises ───────────────────────────────────────────────────────────────────
st.markdown(sec("Remises moyennes par boutique (%)",
    "High discount can signal promotions or pricing strategy."), unsafe_allow_html=True)
sh4 = shops.sort_values("remise_moyenne", ascending=False)
fig5 = px.bar(sh4, x="shop_name", y="remise_moyenne", color="remise_moyenne",
    text=sh4["remise_moyenne"].round(1),
    color_continuous_scale=["#1a1a28", "#3d2b1a", "#ffd93d"],
    template="plotly_dark", title="Remise moyenne (%)")
fig5.update_traces(textposition="outside")
fig5.update_layout(**PLOT, height=350, showlegend=False,
    coloraxis_showscale=False, xaxis_tickangle=-30)
st.plotly_chart(fig5, use_container_width=True)

# ── Tableau complet ───────────────────────────────────────────────────────────
st.markdown(sec("Tableau récapitulatif complet", "Complete boutique data — all metrics."), unsafe_allow_html=True)
st.dataframe(shops.sort_values("score_moyen", ascending=False).reset_index(drop=True),
