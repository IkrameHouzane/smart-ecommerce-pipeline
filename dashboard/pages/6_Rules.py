...
# ── Page title ────────────────────────────────────────────────────────────────
st.markdown('<div class="pg-main">Association Rules</div>'
...
c2.markdown(kpi(f"{n_itemsets:,}",              "Itemsets fréquents", "#ffd93d"), unsafe_allow_html=True)
c3.markdown(kpi(f"{rules['lift'].max():.2f}",   "Lift maximum",       "#4ecdc4"), unsafe_allow_html=True)
c4.markdown(kpi(f"{rules['confidence'].mean():.1%}","Confiance moy.", "#e8d5a3"), unsafe_allow_html=True)

# ── Insight narrative ─────────────────────────────────────────────────────────
top_rule = rules.nlargest(1, "lift").iloc[0]
st.markdown(ins("Strongest Association",
    ...
    "#ff6b6b"), unsafe_allow_html=True)

# ── Filtres interactifs ───────────────────────────────────────────────────────
st.markdown(sec("Filtres interactifs", "Adjust thresholds to explore the rule space."), unsafe_allow_html=True)
...
st.markdown(sec("Lift vs Confiance",
    "Each bubble = one rule. Size = support (frequency). Top-right = most interesting rules."), unsafe_allow_html=True)
fig = px.scatter(filtered, x="confidence", y="lift", size="support", color="lift",
    ...
    opacity=0.85, labels={"confidence": "Confidence", "lift": "Lift"})
...
# ── Top règles par lift ───────────────────────────────────────────────────────
st.markdown(sec("Top règles par lift", "Sorted by lift descending."), unsafe_allow_html=True)
...
st.markdown(sec("Distribution du lift et de la confiance",
    "Histogram of all rules — most cluster at lift 2–4 with high confidence."), unsafe_allow_html=True)
c1, c2 = st.columns(2)
with c1:
    fig2 = px.histogram(rules, x="lift", nbins=40,
        color_discrete_sequence=["#ff6b6b"], template="plotly_dark",
        title="Distribution du lift (toutes règles)", labels={"lift": "Lift"})
    fig2.update_layout(**PLOT, height=320)
    st.plotly_chart(fig2, use_container_width=True)
with c2:
    fig3 = px.histogram(rules, x="confidence", nbins=30,
        color_discrete_sequence=["#ffd93d"], template="plotly_dark",
        title="Distribution de la confiance", labels={"confidence": "Confidence"})
