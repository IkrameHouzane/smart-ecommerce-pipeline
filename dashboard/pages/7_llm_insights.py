...
def build_context() -> dict:
    ...
    cats = (
        scored["category_clean"].value_counts().head(5).to_dict()
        if "category_clean" in scored.columns
        else {}
    )
    km = cls_stats["kmeans"]
    cd = km["clusters"]
...
def log_usage(source: str, prompt_preview: str, response_preview: str):
    try:
        ...
        with open(
            os.path.join(ANALYTICS, "llm_usage_log.jsonl"), "a", encoding="utf-8"
        ) as f:
...
SYNTHESES = {
    ...
    "⚔️ Analyse concurrentielle": (
        ...
        lambda ctx: COMPETITIVE_ANALYSIS_PROMPT.format(
            data=json.dumps(
                {
                    "boutiques": ctx.get("classement_boutiques", []),
...
if st.session_state.chat_history:
    st.markdown(
        '<div style="margin:16px 0 8px 0;border-top:1px solid #1a1a28;padding-top:16px;"></div>',
        unsafe_allow_html=True,
    )
    for msg in st.session_state.chat_history:
        if msg["role"] == "user":
            st.markdown(
                f'<div class="chat-lbl-u">Vous</div><div class="chat-user">{msg["content"]}</div>',
                unsafe_allow_html=True,
            )
        else:
            import re
...
st.markdown(
    sec(
        "Enrichissement automatique de produit",
...
# ═══════════════════════════════════════════════════════════════════════════════
st.markdown(
    sec("Architecture responsable — MCP", "Model Context Protocol d'Anthropic"),
    unsafe_allow_html=True,
)
...
if os.path.exists(mcp_file):
    st.markdown(
        ins(
            "Implémentation MCP",
            "Le fichier <code>mcp/architecture.py</code> contient l'implémentation complète : "
            "<strong>AnalyticsReaderServer</strong> (whitelist fichiers, permission check, log DENIED), "
            "<strong>SummaryGeneratorServer</strong> (génération LLM depuis agrégats uniquement), "
            "<strong>MCPClient</strong> (routage Host→Servers), "
            "<strong>_log_access()</strong> (audit trail append-only avec timestamp UTC).",
            "#a8e6cf",
        ),
        unsafe_allow_html=True,
    )
