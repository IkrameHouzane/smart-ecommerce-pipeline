...
# ── Page title ────────────────────────────────────────────────────────────────
st.markdown(
    f'<div class="pg-main">Segmentation</div>'
...
c1.markdown(
    kpi(f"{kmeans_stats['silhouette_score']:.3f}", "Silhouette Score", "#4ecdc4"),
    unsafe_allow_html=True,
)
c2.markdown(
    kpi(f"K = {kmeans_stats['k']}", "Clusters KMeans", "#e8d5a3"),
    unsafe_allow_html=True,
)
c3.markdown(
    kpi(f"{dbscan_stats['nb_anomalies']}", "Anomalies DBSCAN", "#ff6b6b"),
    unsafe_allow_html=True,
)
c4.markdown(
    kpi(f"{pca_stats['variance_expliquee'] * 100:.1f}%", "Variance PCA", "#a8e6cf"),
    unsafe_allow_html=True,
)
...
st.markdown(
    sec(
        "Visualisation KMeans — PCA 2D",
        f"2D projection — {pca_stats['variance_expliquee'] * 100:.1f}% variance explained. "
        f"Each dot is a product, colored by cluster assignment.",
    ),
    unsafe_allow_html=True,
)
...
fig = px.scatter(
    ...
    y="pca_y",
    color="cluster_label",
    color_discrete_sequence=PAL_CLU,
    template="plotly_dark",
    title="Clusters KMeans en espace PCA 2D",
    opacity=0.65,
...
st.markdown(
    sec(
        "Profil des clusters",
        "Business interpretation of each segment — price, type, and discount signal.",
    ),
    unsafe_allow_html=True,
)
...
st.markdown(
    sec("Distribution des clusters", "Product count and positioning by segment."),
    unsafe_allow_html=True,
)
c1, c2 = st.columns(2)
with c1:
    ...
    fig2 = px.bar(
        x=[f"Cluster {i}" for i in dist.index],
        y=dist.values,
        color=dist.values,
        text=dist.values,
...
with c2:
        ...
        [
            {
                ...
                "Prix moyen": d["prix_moyen"],
                "Score moyen": d["score_moyen"],
                "Note": d["note_moyenne"],
            }
    ...
    fig3 = px.scatter(
        ...
        color_discrete_sequence=PAL_CLU,
        template="plotly_dark",
        title="Prix moyen vs Score moyen par cluster",
        size_max=40,
    )
...
disp = [
    c
    for c in ["title", "shop_name", "price", "composite_score"]
    if c in anomalies.columns
]
st.dataframe(
    anomalies[disp].sort_values("price", ascending=False).reset_index(drop=True),
