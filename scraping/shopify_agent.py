...
def scraper_boutique_shopify(store):
    ...
    while True:
        url = (
            f"{store['url']}/products.json?limit={PRODUCTS_PER_PAGE}&page={numero_page}"
        )
        ...
        try:
            ...
            if reponse.status_code == 403:
                print("   ⚠️  403 — accès refusé par la boutique")
                break
            if reponse.status_code == 404:
                print("   ⚠️  404 — boutique non Shopify ou URL incorrecte")
                break
            ...
            if not produits:
                print("   ✅ Dernière page atteinte — scraping terminé")
                break
            ...
            for p in produits:
                ...
                produit = {
                    ...
                    "description": nettoyer_html(safe(p.get("body_html")))[:500],
