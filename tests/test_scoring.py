...
def top_k(df: pd.DataFrame, k: int = 3, score_col: str = "composite_score") -> pd.DataFrame:
...
class TestScoringFormula:
    ...
    def test_handles_missing_values(self):
        ...
        scores = compute_score(df)
        assert not scores.isna().any(), "Aucun score NaN même avec des valeurs manquantes"
...
class TestTopKRanking:
    ...
    def test_top_k_sorted_descending(self, sample_products_df):
        ...
        assert scores == sorted(scores, reverse=True), "Top-K doit être trié par score décroissant"

    def test_top_1_is_best_product(self, sample_products_df):
        result = top_k(sample_products_df, k=1)
        assert result.iloc[0]["composite_score"] == sample_products_df["composite_score"].max()
